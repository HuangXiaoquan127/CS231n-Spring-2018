...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        A1, fc1_relu1_cache = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        ...
        dA1, grads['W2'], grads['b2'] = affine_backward(dZ2, fc2_cache)
        dX, grads['W1'], grads['b1'] = affine_relu_backward(dA1, fc1_relu1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        hidden_dim_last = input_dim
        for i, hidden_dim in enumerate(hidden_dims):
            self.params['W'+ str(i+1)] = np.random.normal(0.0, weight_scale, (hidden_dim_last, hidden_dim))        
...
#         print(self.num_layers)
        for i in range(self.num_layers - 1):          
            if self.normalization==None :
                A, fc_relu_cache_temp = affine_relu_forward(A, self.params['W' + str(i+1)], self.params['b' + str(i+1)])
                fc_relu_cache.append(fc_relu_cache_temp)                

            elif self.normalization=='batchnorm':
                A, cache_temp = affine_bn_relu_forward(A, self.params['W' + str(i+1)], self.params['b' + str(i+1)], 
                                          self.params['gamma' + str(i+1)], self.params['beta' + str(i+1)], self.bn_params[i]) 
                cache.append(cache_temp)
            elif self.normalization=='layernorm':
                A, cache_temp = affine_ln_relu_forward(A, self.params['W' + str(i+1)], self.params['b' + str(i+1)], 
                                          self.params['gamma' + str(i+1)], self.params['beta' + str(i+1)], self.bn_params[i]) 
                cache.append(cache_temp)
            
            if self.use_dropout:
                A, dropout_cache_temp = dropout_forward(A, self.dropout_param)
                dropout_cache.append(dropout_cache_temp)
            
        Z, fc_cache = affine_forward(A, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        scores = Z
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dA, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = affine_backward(dZ, fc_cache)
        for j in range(self.num_layers - 1):
            i = (self.num_layers - 1) - 1 - j
            if self.use_dropout:
                dA = dropout_backward(dA, dropout_cache[i])
            
            if self.normalization==None :
                dA, grads['W' + str(i+1)], grads['b' + str(i+1)] = affine_relu_backward(dA, fc_relu_cache[i])
            elif self.normalization=='batchnorm':
                dA,grads['W'+str(i+1)],grads['b'+str(i+1)],grads['gamma'+str(i+1)],grads['beta'+str(i+1)]=affine_bn_relu_backward(dA, cache[i])
            elif self.normalization=='layernorm':
                dA,grads['W'+str(i+1)],grads['b'+str(i+1)],grads['gamma'+str(i+1)],grads['beta'+str(i+1)]=affine_ln_relu_backward(dA, cache[i])
